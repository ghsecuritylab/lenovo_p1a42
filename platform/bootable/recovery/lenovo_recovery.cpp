...
/* Begin, lenovo-sw wangxf14 porting lenovo factory ####7777# */
#ifdef LENOVO_FACTORY_WIPE_DATA_SHUTDOWN
  { "factory_wipe_data", no_argument, NULL, 'a' },
#endif
/* End, lenovo-sw wangxf14 porting lenovo factory ####7777# */
#ifdef LENOVO_OTA_AUTO_TEST
  { "auto_ota_upgrade", no_argument, NULL, 'z' },
  { "adb_on", no_argument, NULL, 'b' },
#endif
  { NULL, 0, NULL, 0 },
};
...
static const char *CLEAR_FLAG_FILE = "/cache/recovery/.clearflag";
...
/*Begin, lenovo-sw wangxf14 porting lenovo recovery*/
static const char *EMMC_UPDATE_DEFAULT_FILE = "/sdcard/update.zip";
static const char *SD_UPDATE_DEFAULT_FILE = "/sdcard2/update.zip";
static const char *LENOVO_OTA_RESULT_FILE_ENCRYPT = "/cache/ota/updateResult";
...
/*End, lenovo-sw wangxf14 porting lenovo recovery*/
...
char* reason = NULL;
#ifdef LENOVO_OTA_AUTO_TEST
int auto_ota_upgrade = 0;
int adb_on = 0;
#endif

int need_clear_flag = 0;
...
/* Begin, lenovo-sw wuwl9 20150618 modify for ui language auto recognition */
int get_language_flag(void)
{
    #define MAX_FILE_GETPROP_SIZE    65536
	int ret;
    char* filename = "/data/property/persist.sys.language";
    ...
    if(ensure_path_mounted("/data")== 0)
    {
        ...
        {
            printf("[lenovo]failed to stat %s \n",filename);
            fprintf(stdout,"[lenovo]error now is: %s \n",strerror(errno));
            goto error;
        }
        if (st.st_size > MAX_FILE_GETPROP_SIZE) {
            printf("[lenovo] %s too large for max  65536 \n",filename);
            goto error;
        }
        language_str =(char*)malloc(st.st_size+1);
        if (language_str == NULL) {
             printf("[lenovo] failed to alloc %s \n",language_str);
            goto error;
        }

        f = fopen(filename, "rb");
        if (f == NULL) {
            printf(" [lenovo]failed to open %s \n",filename);
            goto error;
        }
        if (fread(language_str, 1, st.st_size, f) != st.st_size) {
            printf("[lenovo] failed to read %s \n",filename);
            fclose(f);
            goto error;
        }
        language_str[st.st_size] = '\0';
        fclose(f);
        printf("[lenovo]%s is %s\n", filename, language_str);
        if (!strncmp(language_str,"n/a",3)) {
            printf("[lenovo]Failed to get /data/property/persist.sys.language\n");
            goto error;
        }
        if (!strncmp(language_str,"zh",2))
        {
            printf("[lenovo]persist.sys.language is zh \n");
            ret = 0;
            goto success;
        }
        else if(!strncmp(language_str,"en",2))
        {
            printf("[lenovo]persist.sys.language is en \n");
		ret = 1;
            goto success;
        }
	else
        {
            printf("[lenovo]others persist.sys.language value\n");
		ret = 0;
	        goto error;
        }
        ensure_path_unmounted("/data");
    ...
    {
        printf("[lenovo]in [get_language_flag] /data mount failed\n");
        goto error;
    }
...
success:
      return ret;
error:
    ret = 1; //default use English in row product
    if (language_str)
    {
      free(language_str);
    }
...
/* End, lenovo-sw wuwl9 20150618 modify for OTA update cartoon */
...
copy_logs() {
    ...
    chmod(CLEAR_FLAG_FILE, 0600);
    chown(CLEAR_FLAG_FILE, 1000, 1000);   // system user
...
void finish_recovery(const char *send_intent) {
    ...
    if (need_clear_flag == 1) {
        FILE* fp = fopen_path(CLEAR_FLAG_FILE, "w");
        if(fp == NULL) {
            LOGE("Can't open \"%s\"\n", CLEAR_FLAG_FILE);
        } else {
            LOGI("Creating cflag\n");
            fwrite("clear", 1, strlen("clear"), fp);
            fflush(fp);
            fsync(fileno(fp));
            check_and_fclose(fp, CLEAR_FLAG_FILE);
        }
    }
	...
	strlcpy(boot.recovery, "recovery\n", sizeof(boot.recovery));
	strlcat(boot.recovery, "--wipe_data_only\n", sizeof(boot.recovery));
	strlcat(boot.recovery, "--adb_on\n", sizeof(boot.recovery));
	ui_print("auto_test_tool set command wipe_adb_on \n");
    }
#endif	
    set_bootloader_message(&boot);
	...
	if(ensure_path_mounted("/data") == 0)
	{
	    if(mkdir("/data/property", 0700) == 0)
	    {
	        int ret=0;
	        printf("shenjun1 mount midir successfully \n");
	        ret = property_set("persist.sys.usb.config", "mtp,adb");
	        printf("set property persist.sys.usb.config = %d\n", ret);
	        ret = property_set("persist.sys.usb.factorymode", "true");
	        printf("set property persist.sys.usb.factorymode = %d\n", ret);
	        sync();
	    }
	    else
	    {
	        ui_print("auto_test_tool set adb_on, create dir property failed \n");
	    }
	    ensure_path_unmounted("/data");
	}
	else
	{
	    printf("shenjun1 mount data failed \n");
	    ui_print("auto_test_tool set adb_on, mount data partition failed \n");
	}
    }
#endif	
...
#endif // lenovo-sw wangxf14 debug #if 0
...
{
/* lenovo-sw wangxf14 porting lenovo recovery
...
//    ensure_path_mounted("/data");
    ...
    *p = 0;
	mode_t mask = umask(0);//Lenovo-sw zhucc2 add

	//Lenovo-row, miaotao1, Start
	chmod(dir_name,0777);
	chmod(file_name,0666);
	//Lenoov-row, miaotao1, End
    ...
    if (opendir(dir_name) == NULL) {
        ...
        if (mkdir(dir_name, 0777))  {//lenovo-sw wangxf14 modify in 20130916, modify for ota apk miss ota update result on single new path
	        ...
	        umask(mask);//Lenovo-sw zhucc2 add
            return;
        }
...
//begin, lenovo sw wuwl9 add for dev->st OTA 20151121
int Get_ChangeDevToSt_Flag()
{
    #define MAX_FILE_GETPROP_SIZE    65536
	int ret;
    char* filename_cache = "/cache/lenovorollback/rollback";
    int length ;
    char* rollback_str = NULL;
    FILE* f ;
    struct stat st;
    printf("[lenovo]in to Get_ChangeDevToSt_Flag() \n");
 if(ensure_path_mounted("/cache")== 0)
    {

        printf("[lenovo]in get_rollback_flag()   go into mount /cache.\n");
        if (stat(filename_cache, &st) < 0)
        {
            printf("[lenovo]failed to stat %s \n",filename_cache);
            fprintf(stdout,"[lenovo]error now is: %s \n",strerror(errno));
            goto error;
        ...
        rollback_str =(char*)malloc(st.st_size+1);
        if (rollback_str == NULL) {
             printf("[lenovo] failed to alloc %s \n",rollback_str);
            goto error;
        }

        f = fopen(filename_cache, "rb");
        if (f == NULL) {
            printf(" [lenovo]failed to open %s \n",filename_cache);
            goto error;
        }
        if (fread(rollback_str, 1, st.st_size, f) != st.st_size) {
            printf("[lenovo] failed to read %s \n",filename_cache);
            fclose(f);
            goto error;
        }
        rollback_str[st.st_size] = '\0';
        fclose(f);
        printf("[lenovo]%s is %s\n", filename_cache, rollback_str);
        if (!strncmp(rollback_str,"n/a",3)) {
            printf("[lenovo]Failed to get valid /cache/lenovorollback/rollback\n");
            goto error;
        }
        else if(!strncmp(rollback_str,"0",1))
        {
            printf("[lenovo]get_rollback_flag() get /cache/lenovorollback/rollback is 0 \n");
		    ret = 1;
            goto success;
        }
	else
        {
            printf("[lenovo]get_rollback_flag() get /cache/lenovorollback/rollback is not 0\n");
		    ret = 0;
	        goto error;
        }
    ...
    {
        printf("[lenovo]in Get_ChangeDevToSt_Flag()   /cach mount failed\n");
        goto error;
    }
success:
      return ret;
...
//end, lenovo sw wuwl9 add for dev->st OTA 20151121
...
{
    write_result_file(LENOVO_OTA_RESULT_FILE_ENCRYPT, status);
...
int lenovo_fuse_wipe_clean(const char *volume)
{
    ...
    if (ensure_path_mounted("/data") == 0)
        ...
        else
        {
            ...
            ret = erase_volume("/data");
             if( 0 == ret )
                ret = erase_volume("/cache");
        }
#else
        printf("anroid L can only do format\n");
        ret = erase_volume("/data");
		if( 0 == ret )
			ret = erase_volume("/cache");
#endif
...
/* Begin, lenovo-sw wuwl9 20150604 add for fixed PASION-2118 workaround lcd flick  */
...
/* End, lenovo-sw wuwl9 20150604 add for fixed PASION-2118 workaround lcd flick  */
...
static intentResult* intent_install(int argc, char *argv[])
{
    ...
    miuiInstall_init(&install_package, (const char *)argv[0], wipe_cache, TEMPORARY_INSTALL_FILE, 1);
...
int is_full_update_zip(const char *dir) {
    ...
    if (package_data) {
        if (package_data[0] == '@') {
            if (ensure_path_mounted(package_data+1) != 0) {
                LOGE("Can't mount %s\n", package_data);
                return INSTALL_CORRUPT;
            }
        } else {
            if (ensure_path_mounted(package_data) != 0) {
                LOGE("Can't mount %s\n", package_data);
                return INSTALL_CORRUPT;
            }
        }
    ...
    MemMapping map;
    if (sysMapFile(package_data, &map) != 0) {
...
/* lenovo-sw wangxf14 porting lenovo factory */
#ifdef LENOVO_FACTORY_WIPE_DATA_SHUTDOWN
...
#ifdef LENOVO_FACTORY_WIPE_DATA_SHUTDOWN
        case 'a': ex_wipe_data = wipe_cache = 1; factory_wipe_data = 1; need_clear_flag = 1; break;//lenovo-sw zhucc2 modify for ####7777# not restared after master clear
#endif
/* lenovo-sw chentao8 add for ####8888# not restared after master clear end */
#ifdef LENOVO_OTA_AUTO_TEST
	case 'z': auto_ota_upgrade = 1; break;
	case 'b': adb_on = 1; break;
#endif
        case '?':
...
//Begin, lenovo-sw wangxf14 add for solution of exfat mount system that cause diff ota update error
    ensure_path_unmounted("/system");
//End, lenovo-sw wangxf14 add for solution of exfat mount system that cause diff ota update error
    device_ui_init();
...
#ifdef LENOVO_SHARED_SDCARD
  /* lenovo-sw : add for ####7777# , Remove content of /data partition excluding data/media files */
    printf("wipe_data = %d, wipe_cache = %d, ext_wipe_data =%d, need_clear_flag =%d \n", wipe_data, wipe_cache, ex_wipe_data, need_clear_flag);
  /* lenovo-sw : add for ####7777# , Remove content of /data partition excluding data/media files */
...
/* Begin, lenovo-sw wangxf14 20130912 add, add for protect suddenness that phone power off in recovery system operate */
        ...
        if (wipe_cache)
        {
            ...
            if (factory_wipe_data == 1)
                miuiIntent_send(INTENT_WIPE_LENOVO, 3, "9", "/cache", "/data");//* lenovo-sw zhucc2 modify for ####7777# not restared after master clear
            else
#endif
            miuiIntent_send(INTENT_WIPE_LENOVO, 3, "0", "/cache", "/data");
            status = miuiIntent_result_get_int();
