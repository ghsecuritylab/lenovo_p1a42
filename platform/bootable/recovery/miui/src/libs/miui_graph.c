...
/* Begin, lenovo-sw wangxf14 20150403 add, add for workaround lcd flick  */
...
/* End, lenovo-sw wangxf14 20150403 add, add for workaround lcd flick  */
  ...
  if (ag_fb>0){
    ...
    if (ag_fbv.bits_per_pixel==16){
     /*RGB565*/
      ...
      ag_32   = 0;
	  //lenovo_debug("lenovo debug lcd parameter init(ag_32 = 0)!\n");
	  ...
	  int write_return;
      ag_32     = 1;
	 // lenovo_debug("lenovo debug lcd parameter init(ag_32 = 1)!\n");
	  ret = read_file_int("/sys/class/leds/lcd-backlight/brightness", &tmp);
	  printf("[lenovo]before set/sys/class/leds/lcd-backlight/brightness is %d\n", tmp);

	  write_return = write_file("/sys/class/leds/lcd-backlight/brightness", "30");
	  fprintf(stdout,"[lenovo]return value is :%d ; error now is :%s \n ",write_return , strerror(errno));
	  ret = read_file_int("/sys/class/leds/lcd-backlight/brightness", &tmp);
	  printf("[lenovo]after set /sys/class/leds/lcd-backlight/brightness is %d\n", tmp);
...
//-- Refresh Thread
static void *ag_thread(void *cookie){
  miui_debug("ag_isrun = %d ag_isbusy = %d\n", ag_isrun, ag_isbusy);
  while(ag_isrun){
    if(ag_isbusy!=2){
      usleep(166000);
      miui_debug("longdelay ag_isrun = %d, ag_isbusy = %d, ag_refreshlock = %d\n", ag_isrun, ag_isbusy, ag_refreshlock);
      if (!ag_isrun)
      {
          break;
      }
      ...
      usleep(16600);
      miui_debug("shortdelay ag_isrun = %d, ag_isbusy = %d, ag_refreshlock = %d\n", ag_isrun, ag_isbusy, ag_refreshlock);
      if (!ag_isrun) break;
...
void lenovo_ag_copybusy(char * wait)
{
  ...
  {
    miui_debug("spinner progress get png success spinner_ap.w = %d, spinner_ap.h = %d\n", spinner_ap.w, spinner_ap.h);
  ...
  {
    miui_debug("spinner progress get png failured\n");
  }

  miui_debug("spinner progress ag_spinner_ox = %d, ag_spinner_oy = %d, ag_spinner_ow = %d, ag_spinner_oh = %d\n", ag_spinner_ox, ag_spinner_oy, ag_spinner_ow, ag_spinner_oh);
...
void ag_busyprogress(){
  ...
  miui_debug("ag_busypos = %d, bs_x = %d, bs_y = %d, bs_h = %d, bs_w = %d, bs_w2 = %d\n", ag_busypos, bs_x, bs_y, bs_h, bs_w, bs_w2);
  miui_debug("ag_32 = %d, agclp = %d\n", ag_32, agclp);
...
void ag_refreshrate(){
  ...
  miui_debug("ag_refreshrate doing ... ag_32 = %d\n", ag_32);

  //-- Copy Data
  if (ag_32==1){
    if (ag_isbusy==0){
      miui_debug("ag_isbusy==0\n");
      ag32fbufcopy(ag_bf32);
      //memcpy(ag_fbuf32,ag_bf32,ag_fbsz);
    }
    else if(ag_isbusy==2){
      miui_debug("ag_isbusy==2\n");
      ag32fbufcopy(ag_bz32);
...
//    else if(ag_lastbusy<alib_tick()-50 * 1000){
      ...
      if( 1 == first_enter )
      {
        miui_debug("first enter wait!\n");
		first_enter = 0;
		ag_lastbusy = alib_tick();
      }
      else
      {
          miui_debug("debug ag_lastbusy<alib_tick()-50\n");
#ifdef LENOVO_SPINNER_PROGRESS
...
//  ioctl(ag_fb, FBIOPUT_VSCREENINFO, &ag_fbv);
  ...
  if( workaround_lcd_flicker_count == 1 )
  {
    write_file("/sys/class/leds/lcd-backlight/brightness", "0");
	ioctl(ag_fb, FBIOPUT_VSCREENINFO, &ag_fbv);
  }
  else if ( workaround_lcd_flicker_count == 2 )
  {
    write_file("/sys/class/leds/lcd-backlight/brightness", "30");
	ioctl(ag_fb, FBIOPUT_VSCREENINFO, &ag_fbv);
  }
  else
  {
...
/*
  ...
  {
    write_file("/sys/class/leds/lcd-backlight/brightness", "104");
  }
*/
}
...
void ag_sync(){
  //-- Always On Footer
  // ag_draw_foot();
  miui_debug("ag_refreshlock = %d, ag_isbusy = %d, ag_sync_locked = %d\n", ag_refreshlock, ag_isbusy, ag_sync_locked);
  ag_isbusy = 0;
  if (!ag_sync_locked){
    ag_refreshlock=1;
    if (ag_32==1){
...
void ag_sync_force(){
  miui_debug("wangxf14_look ag_sync_locked = %d\n", ag_sync_locked);
  if (ag_sync_locked)
    ag_sync_locked = 0;
  else
    ag_sync();
}
static void *ag_sync_fade_thread(int cookie){
...
void ag_sync_fade_wait(int frame){
  ag_sync_fade_thread(frame);
  return;
}
void ag_sync_fade(int frame){
  pthread_t threadsyncfade;
  pthread_create(&threadsyncfade,NULL, ag_sync_fade_thread, frame);
