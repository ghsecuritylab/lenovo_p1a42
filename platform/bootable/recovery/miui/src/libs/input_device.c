...
static byte aipInitEventDev(AIP_EVP e){
  ...
  //lenovo-sw wuwl9 add for VIBEUIDEVL-789 disabled unnecessary device's massege
  //lenovo-sw wangxf14 modify for p1
  if ((strcmp(e->device_name, "compass") == 0)
  		||(strcmp(e->device_name, "input_mt_wrapper") == 0)
  		||(strcmp(e->device_name, "accelerometer") == 0)
  		||(strcmp(e->device_name, "proximity") == 0)
  		||(strcmp(e->device_name, "light") == 0)
  		)
  	{
      e->ignored = 1;
  	}
