...
static int ai_cartoon_frames = 18;
static int gCartoonFrame = 0;
...
static void *miui_install_package(void *cookie){
    ...
    {
        //run install process
        ret = pmiui_install->pfun(pmiui_install->path, &pmiui_install->wipe_cache, pmiui_install->install_file, pmiui_install->needs_mount);
...
static void *ac_lenovo_progressthread(void *cookie)
{
 ...
 /* Begin, lenovo-sw wuwl9 20150618 modify for OTA update cartoon */
	char cartoonname[40];
	int tmp;
	tmp =  gCartoonFrame++ % ai_cartoon_frames;
	sprintf(cartoonname, "img.icon.install.normal%02d", tmp+1);
	miui_debug("[lenovo] Now gCartoonFrame = %d ; tmp = %d ; cartoonname = %s. \n",gCartoonFrame,tmp,cartoonname);
    if(0==atheme_draw(cartoonname, &tmpCanvas, 0, 0,get_ac_update_text_imgW(ai_update), get_ac_update_text_imgH(ai_update)))
    {
        miui_debug("[lenovo]draw cartoon %s failure!\n",cartoonname);
    }
	else
	{
		miui_debug("[lenovo]draw cartoon %s succes!\n",cartoonname);
	}
/* End, lenovo-sw wuwl9 20150618 modify for OTA update cartoon */
    color txtcolor = ag_rgb(0xff,0xff,0xff);
    ag_texts (&tmpCanvas,ptxt_w,ptxt_x,ptxt_y,prog_percent_str,txtcolor, 1);
...
STATUS miuiInstall_init(miuiInstall_fun fun, const char* path, int wipe_cache, const char* install_file, int needs_mount)
{
	...
	pmiui_install->needs_mount = needs_mount;
    return RET_OK;
}
...
/* lenovo-sw zhucc2 port for ####7777# not restared after master clear begin */
...
#endif
/* lenovo-sw zhucc2 port for ####7777# not restared after master clear end */
