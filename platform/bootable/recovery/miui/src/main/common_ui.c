...
/* Begin, lenovo-sw wuwl9 20150618 modify for ui language auto recognition */
	if( 0 == get_language_flag() )
		{
			printf("[lenovo] set language to china in commom_button_ui_show \n");
		...
		{
			printf("[lenovo] set language to english in commom_button_ui_show \n");
			miui_loadlang("langs/en.lang");
        	miui_font( "0", "ttf/DroidSans.ttf", "18" );
        	miui_font( "1", "ttf/DroidSans.ttf", "24" );
		}
/* End, lenovo-sw wuwl9 20150618 modify for ui language auto recognition */
    int n = p->get_child_count(p);
    if (n == 0) return common_ui_show_node(p);//show node
    int selindex = 0;
    return_val_if_fail(n >= 1, RET_FAIL);
