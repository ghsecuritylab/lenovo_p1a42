...
typedef struct {
    ...
    int p_block;
    size_t p_remain;
//begin,lenovo-sw wuwl9 20150707 modify for auto progress bar for  PASION-3878&PASION-3989
	FILE* cmd_pipe;
	double rss_bar_position;
	char* rss_cmdname;
	int rss_fullota_flag;
//end,lenovo-sw wuwl9 20150707 modify for auto progress bar for  PASION-3878&PASION-3989
} RangeSinkState;

static ssize_t RangeSinkWrite(const uint8_t* data, ssize_t size, void* token) {
    RangeSinkState* rss = (RangeSinkState*) token;
	double bar_position;//lenovo-sw wuwl9 20150707 modify for auto progress bar for  PASION-3878&PASION-3989
    if (rss->p_remain <= 0) {
    ...
    while (size > 0) {
        ...
        if (rss->p_remain == 0) {
		//begin,lenovo-sw wuwl9 20150707 modify for auto progress bar for  PASION-3878&PASION-3989
		printf("[lenovo]===========rss.rss_cmdname = %s  ; rss_fullota_flag =%d  ==========\n",rss->rss_cmdname,rss->rss_fullota_flag);
		//if (rss->rss_cmdname[0] == 'n')rss.rss_fullota_flag
		if (rss->rss_fullota_flag == 1)
			{
				//bar_position = rss->rss_bar_position + (1-rss->rss_bar_position)*(double)(rss->p_block/33.0);
				bar_position = rss->rss_bar_position + (1.0-rss->rss_bar_position)*(((double)rss->p_block)/((double)rss->tgt->count));
				printf("[lenovo]prog_bar_position=%.2f;rss->p_block=%d ;block_per= %.2f;bar_position=%.2f;rss->tgt->count= %d  \n",
					rss->rss_bar_position,rss->p_block,(((double)rss->p_block)/((double)rss->tgt->count)),bar_position,rss->tgt->count);
				if(bar_position>1.0) bar_position=1.0;
				if(bar_position<0.0) bar_position=0.0;
				printf("[lenovo]set_progress :bar_position=%.2f  \n",bar_position);
				fprintf(rss->cmd_pipe, "set_progress %.2f\n", bar_position);
				fflush(rss->cmd_pipe);
			}
		//end,lenovo-sw wuwl9 20150707 modify for auto progress bar for  PASION-3878&PASION-3989
            ...
            if (rss->p_block < rss->tgt->count) {
                ...
                if (rss->rss_fullota_flag == 1)
                	{
                		printf("[lenovo]  DONE,rss->p_block=%d ; rss->tgt->count=%d\n",rss->p_block,rss->tgt->count);
		                bar_position=1.0;
						printf("[lenovo]  DONE,set_progress = %.2f\n",bar_position);
						fprintf(rss->cmd_pipe, "set_progress %.2f\n", bar_position);
						fflush(rss->cmd_pipe);
                	}
				//end,lenovo-sw wuwl9 20150707 modify for auto progress bar for  PASION-3878&PASION-3989
...
//begin,lenovo-sw wuwl9 20150707 modify for auto progress bar for  PASION-3878&PASION-3989
	FILE* cmd_pipe;
	double newthread_bar_position;
	int newthread_fullota_flag;
//end,lenovo-sw wuwl9 20150707 modify for auto progress bar for  PASION-3878&PASION-3989
} NewThreadInfo;
...
static int CreateStash(State* state, int maxblocks, const char* blockdev, char** base) {
    ...
    *base = PrintSha1(digest);
    ...
    if (res == -1 && errno != ENOENT) {
		...
		fprintf(stderr, "[wuwl9]creating stash %s, ret return = %d, error now is :%s\n", dirname,res,strerror(errno));

        if (res != 0) {
			fprintf(stderr,"[wuwl9]failed to create %s \n", dirname);
			res = mkdir("/cache/recovery", STASH_DIRECTORY_MODE);
			sync();
			res = mkdir(dirname, STASH_DIRECTORY_MODE);
			fprintf(stderr, "again creating stash %s, ret return = %d, error now is :%s\n", dirname,res,strerror(errno));
			if (res != 0)
			{
			    fprintf(stderr,"again failed to create %s \n", dirname);
            goto csout;
			}
        }
...
static int PerformCommandNew(CommandParameters* params) {
    ...
    if (params->canwrite) {
        ...
        rss.p_block = 0;
//begin,lenovo-sw wuwl9 20150707 modify for auto progress bar for  PASION-3878&PASION-3989
		rss.cmd_pipe = params->nti.cmd_pipe;
		rss.rss_bar_position = params->nti.newthread_bar_position;
		rss.rss_cmdname = params->cmdname;
		rss.rss_fullota_flag = params->nti.newthread_fullota_flag;
//end,lenovo-sw wuwl9 20150707 modify for auto progress bar for  PASION-3878&PASION-3989
...
static Value* PerformBlockImageUpdate(const char* name, State* state, int argc, Expr* argv[],
    ...
    za = ui->package_zip;
	bar_position = ui->update_bar_position;//begin,lenovo-sw wuwl9 20150707 modify for auto progress bar PASION-3878&PASION-3989
	fullota_flag = ui->fullota_flag;
...
//begin,lenovo-sw wuwl9 20150707 modify for auto progress bar for PASION-3878&PASION-3989
		params.nti.cmd_pipe = cmd_pipe;
		params.nti.newthread_bar_position = bar_position;
		params.nti.newthread_fullota_flag = fullota_flag;
//end,lenovo-sw wuwl9 20150707 modify for auto progress bar for  PASION-3878&PASION-3989
    ...
    for (line = strtok_r(NULL, "\n", &linesave); line;
        ...
        if (params.canwrite) {
			//changed for fixed PASSION-1332
           // fprintf(cmd_pipe, "set_progress %.4f\n", (double) params.written / total_blocks);
           fprintf(stderr, "===== params.cmdname = %s ; fullota_flag = %d ============== ;\n",
           					params.cmdname,fullota_flag);
           if (fullota_flag != 1)
           	{
           	 	fprintf(stderr, "params.written = %d ;total_blocks = %d ;bar_per = %.2f\n",
           					 params.written, total_blocks,(((double)params.written)/((double)total_blocks)));
				diff_bar_position = ui->update_bar_position + (1-ui->update_bar_position)*(((double)params.written)/((double)total_blocks));
				if(diff_bar_position>1.0) diff_bar_position=1.0;
				if(diff_bar_position<0.0) diff_bar_position=0.0;
				printf("[lenovo]set_progress :bar_position=%.2f  \n",diff_bar_position);
				fprintf(cmd_pipe, "set_progress %.2f\n", diff_bar_position);
            fflush(cmd_pipe);
        }
    }
    }
