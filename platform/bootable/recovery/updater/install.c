...
Value* AccessFileFN(const char* name, State* state, int argc, Expr* argv[]) {
    bool access_ok = false;
    if (argc != 1) {
        return ErrorAbort(state, "%s() expects 1 arg, got %d", name, argc);
    }
    char* file_name = NULL;
    if (ReadArgs(state, argv, 1, &file_name) < 0) {
        return NULL;
    }
    ...
    return StringValue(strdup(access_ok == true ? "t" : ""));
}
...
/* Begin, lenovo-sw wuwl9 add, add for lenovo ota rollback */
Value* LenovoCustomOtaRollback(const char* name, State* state, int argc, Expr* argv[])
{
    //ro.build.date.utc that was on the build that the update package was made
    //from
    #define MAX_FILE_GETPROP_SIZE    65536
    char* package_dev_variant;

    //ro.build.date.utc on the current hardware
    char current_dev_variant[PROPERTY_VALUE_MAX];

    //lenovo-sw wuw9 modify
    char* filename = "/data/data/com.lenovo.ota/files/rollback";
    ...
    //transfer the update package ro.build.date.utc
    package_dev_variant = Evaluate(state, argv[0]);
    if (!package_dev_variant) goto error;

    property_get("ro.build.date.utc", current_dev_variant, "n/a");
    if (!strncmp(current_dev_variant,"n/a",3)) {
          uiPrintf(state, "Failed to get valid ro.build.date.utc");
          goto error;
    }

    uiPrintf(state, "update.zip package_dev_variant is %s, devices current_dev_variant is %s\n",
                    package_dev_variant, current_dev_variant);

    if ( strncmp(current_dev_variant, package_dev_variant, strlen(current_dev_variant)) > 0 )
    {
        uiPrintf(state, "current devices is newer than update.zip, possible do lenovo rollback\n");
//        property_get("persist.sys.ota.rollback", ota_rollback_variant, "n/a");
        struct stat st;
        if (stat(filename, &st) < 0)
		{
            uiPrintf(state, "%s: failed to stat \"%s\": %s , go on to stat rollback in /cache file",
                   name, filename, strerror(errno));
            //goto error;
            if (stat(filename_cache, &st) < 0)
            	{
            	 	uiPrintf(state, "%s: failed to stat \"%s\": %s .  stat rollback in /data and /cache file \n",
                   name, filename_cache, strerror(errno));
            goto error;
        }
            if (st.st_size > MAX_FILE_GETPROP_SIZE) {
	            uiPrintf(state, "%s too large for %s (max %d)",
	                   filename_cache, name, MAX_FILE_GETPROP_SIZE);
	            goto error;
	        }

	        buffer = malloc(st.st_size+1);
	        if (buffer == NULL) {
	            uiPrintf(state, "%s: failed to alloc %lld bytes", name, (long long)st.st_size+1);
	            goto error;
	        }
	        FILE* f = fopen(filename_cache, "rb");
	        if (f == NULL) {
	            uiPrintf(state, "%s: failed to open %s: %s",
	                   name, filename_cache, strerror(errno));
	            goto error;
	        }
	        if (fread(buffer, 1, st.st_size, f) != st.st_size) {
	            uiPrintf(state, "%s: failed to read %lld bytes from %s",
	                   name, (long long)st.st_size+1, filename_cache);
	            fclose(f);
	            goto error;
	        }
	        buffer[st.st_size] = '\0';
	        fclose(f);
	        uiPrintf(state, "%s is %s\n", filename_cache, buffer);
			if (!strncmp(buffer,"n/a",3)) {
	            uiPrintf(state, "Failed to get valid persist.sys.ota.rollback\n");
	            goto error;
	        }
	        if (!strncmp(buffer,"0",1)) {
	            uiPrintf(state, "persist.sys.ota.rollback is true\n");
	            goto success;
	        } else {
	            uiPrintf(state, "Invalid persist.sys.ota.rollback value\n");
		    goto error;
	        }
        }
		else
...
Value* LenovoCustomOtaRollbackFormat(const char* name, State* state, int argc, Expr* argv[])
{
    //ro.build.date.utc that was on the build that the update package was made
    //from
    char* package_dev_variant;
    ...
    uiPrintf(state, "update.zip package_dev_variant is %s, devices current_dev_variant is %s\n",
        ...
        uiPrintf(state, "LenovoCustomOtaRollbackFormat flag judge\n");
        struct stat st;
        if (stat(filename, &st) < 0) {
            uiPrintf(state, "%s: failed to stat \"%s\": %s, go on to stat rollback flag in /cache",
                   name, filename, strerror(errno));
            if(stat(filename_cache, &st) < 0)
            	{
            		uiPrintf(state, "%s: failed to stat \"%s\": %s faile to stat rollback flag in /data&/cache",
                   name, filename_cache, strerror(errno));
            goto error;
        }
			else
...
/* End, lenovo-sw wuwl9 add, add for lenovo ota rollback */
...
//Start [LenovoOTA][miaotao1] 2012-11-7 
    ...
    RegisterFunction("access_file", AccessFileFN);
//End [LenovoOTA][miaotao1]

    RegisterFunction("enable_reboot", EnableRebootFn);
    RegisterFunction("tune2fs", Tune2FsFn);
    RegisterFunction("get_device_compatible", ConfirmDevVariant);
    //begin ,lenovo-sw wuwl9 add for dev to st rollback
    RegisterFunction("get_lenovo_custom1", LenovoCustomOtaRollback);
    RegisterFunction("get_lenovo_custom2", LenovoCustomOtaRollbackFormat);
//end ,lenovo-sw wuwl9 add for dev to st rollback
}
